...
!    Charge terms if needed (see routine dirac.f90).
     ...
     if (contains(mattertype,"electric")) then
        ...
        dirac_PiFR(l,:) = dirac_PiFR(l,:) + dirac_q*(ePhi(l,:)*dirac_FI(l,:) - eAr(l,:)*dirac_GI(l,:)/dsqrt(A(l,:)))
        dirac_PiFI(l,:) = dirac_PiFI(l,:) - dirac_q*(ePhi(l,:)*dirac_FR(l,:) - eAr(l,:)*dirac_GR(l,:)/dsqrt(A(l,:)))

        dirac_PiGR(l,:) = dirac_PiGR(l,:) + dirac_q*(ePhi(l,:)*dirac_GI(l,:) - eAr(l,:)*dirac_FI(l,:)/dsqrt(A(l,:)))
        dirac_PiGI(l,:) = dirac_PiGI(l,:) - dirac_q*(ePhi(l,:)*dirac_GR(l,:) - eAr(l,:)*dirac_FR(l,:)/dsqrt(A(l,:)))
...
!    E, S and v to zero.
     ...
     dust_W(l,:)   = 1.d0/dsqrt(abs(1.d0 - A(l,:)*psi4(l,:)*dust_v(l,:)**2))
