!$Header: /usr/local/ollincvs/Codes/OllinSphere-BiB/src/matter/idata_complexghostpulse.f90,v 1.6 2023/10/25 18:11:23 malcubi Exp $

  subroutine idata_complexghostpulse

! ********************************************
! ***   COMPLEX GHOST PULSE INITIAL DATA   ***
! ********************************************
...
!
!
! with rho the energy density of the complex ghost field:
!
!              2       2            4
...
! ********************************************************
! ***   SET UP GAUSSIAN PULSE IN COMPLEX GHOST FIELD   ***
! ********************************************************

! Initial pulse.  Remember that the complex scalar field must be even.

  if (rank==0) then
     print *, 'complexghostpulse initial data not yet implemented.'
     print *
  end if

  if (complexghostprofile=="gaussian") then

     if (complexghostR_r0==0.d0) then
        complexghost_phiR = gaussian(complexghostR_a0,0.d0,complexghostR_s0)
     else
        complexghost_phiR = gaussian(complexghostR_a0,+complexghostR_r0,complexghostR_s0) &
                          + gaussian(complexghostR_a0,-complexghostR_r0,complexghostR_s0)
     end if

     if (complexghostI_r0==0.d0) then
        complexghost_phiI = gaussian(complexghostI_a0,0.d0,complexghostI_s0)
     else
        complexghost_phiI = gaussian(complexghostI_a0,+complexghostI_r0,complexghostI_s0) &
                          + gaussian(complexghostI_a0,-complexghostI_r0,complexghostI_s0)
     end if

! Smooth top-hat profile.

  else if (complexghostprofile=="tophat") then

     if (complexghostR_r0==0.d0) then
        complexghost_phiR = tophat(complexghostR_a0,0.d0,complexghostR_s0,complexghostR_t0)
     else
        complexghost_phiR = tophat(complexghostR_a0,+complexghostR_r0,complexghostR_s0,complexghostR_t0) &
                          + tophat(complexghostR_a0,-complexghostR_r0,complexghostR_s0,complexghostR_t0)
     end if

     if (complexghostI_r0==0.d0) then
        complexghost_phiI = tophat(complexghostI_a0,0.d0,complexghostI_s0,complexghostI_t0)
     else
        complexghost_phiI = tophat(complexghostI_a0,+complexghostI_r0,complexghostI_s0,complexghostI_t0) &
                          + tophat(complexghostI_a0,-complexghostI_r0,complexghostI_s0,complexghostI_t0)
     end if
...
!    The initial time derivatives are set to zero.
  ...
  else if ((k_parameter/=0.d0).and.(complexghost_mass>0.d0)) then
...
!    constraint trivially.
     ...
     complexghost_piR = 0.d0
     complexghost_piI = k_parameter*complexghost_phiR
...
!    Message to screen.
     ...
     if (rank==0) then
        print *, 'Solving initial data for a complex ghost pulse with zero potential'
        print *
     end if
...
!    Fill in local coefficients of linearized equation.
     ...
     if (newr) then
        ...
        CL1 = - smallpi*(complexghost_xiR**2 + complexghost_xiI**2) &
...
!    Message to screen.
     ...
     if (rank==0) then
        print *, 'Solving initial data for a complex ghost pulse with non-zero potential'
        print *
     end if

!    Find complex ghost field potential.
...
!    Fill in local potential (plus time derivative terms).
     ...
     VL = complexghost_V - 0.5d0*(complexghost_piR**2 + complexghost_piI**2)
...
!       Message in case we reached the maximum iteration number.
        ...
        if (iter>=maxiter) then
           print *
           print *, 'Maximum iteration number reached in idata_complexghostpulse.f90.'
...
! ***************
  ...
  end subroutine idata_complexghostpulse
