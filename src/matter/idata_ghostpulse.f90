!$Header: /usr/local/ollincvs/Codes/OllinSphere-BiB/src/matter/idata_ghostpulse.f90,v 1.13 2023/10/25 18:07:03 malcubi Exp $

  subroutine idata_ghostpulse

! ************************************
! ***   GHOST PULSE INITIAL DATA   ***
! ************************************
...
!
! We then give a simple initial profile in the ghost field
...
!
!
! with rho the energy density of the ghost field:
!
!             2         4
! rho  =  - xi / 2 A psi  +  V
!
...
! ***********************************************
! ***   SET UP INITIAL PULSE IN GHOST FIELD   ***
! ***********************************************

! Initial pulse.  Remember that the ghost field must be even.

! Gaussian profile.

  if (ghostprofile=="gaussian") then

     if (ghost_r0==0.d0) then
        ghost_phi = gaussian(ghost_a0,0.d0,ghost_s0)
     else
        ghost_phi = gaussian(ghost_a0,+ghost_r0,ghost_s0) &
                  + gaussian(ghost_a0,-ghost_r0,ghost_s0)
     end if

! Smooth top-hat profile.

  else if (ghostprofile=="tophat") then

     if (ghost_r0==0.d0) then
        ghost_phi = tophat(ghost_a0,0.d0,ghost_s0,ghost_t0)
     else
        ghost_phi = tophat(ghost_a0,+ghost_r0,ghost_s0,ghost_t0) &
                  + tophat(ghost_a0,-ghost_r0,ghost_s0,ghost_t0)
     end if
...
!    Message to screen.
     ...
     if (rank==0) then
        print *, 'Solving initial data for a ghost pulse with zero potential ...'
        print *
     end if
...
!    Message to screen.
     ...
     if (rank==0) then
        print *, 'Solving initial data for ghost pulse with non-zero potential ...'
        print *
     end if

!    Find ghost field potential.
...
!       Message in case we reached the maximum iteration number.
        ...
        if (iter>=maxiter) then
           print *
           print *, 'Maximum iteration number reached in idata_ghostpulse.f90.'
...
! ***************
  ...
  end subroutine idata_ghostpulse
