...
$\Delta^r$ and stored for output in the array \texttt{CDeltar}:
\begin{equation}
...
\item[\texttt{isotropic}] The initial lapse is set to the
  corresponding isotropic lapse for a static black hole (useful for
  testing).
...
\item[\texttt{dr0 (real)}] Spatial interval for base (coarsest) grid (default 1.0).
...
\item[\texttt{boundtype(character)}] Type of outer boundary condition.
  ...
  \texttt{flat}, \texttt{radiative}, \texttt{constraint} (default
  \texttt{constraint}).
